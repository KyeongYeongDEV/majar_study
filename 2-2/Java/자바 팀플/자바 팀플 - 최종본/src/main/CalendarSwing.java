...
//ToDO 여기껴지 수정
    ...
    Sign_Login signLogin = new Sign_Login();
    ...
    public void setDay(){
        ...
        for(int day=1; day<=lastDay; day++){
            JButton lbl;
            if(check[day]){
                lbl = new JButton("<html>" + String.valueOf(day)+ "<br>V</html>" );//라벨선언해주는데 String.value 는 형변환
            }else{
    ...
    public void setD_day(){
        ...
        try {
            String sql = "SELECT * FROM post WHERE post_team = '" +selectGroup +"' AND post_year >= '"+curYear+"'"  + " ORDER BY post_year , post_month, post_day, idx ASC";
            //+"' AND post_month >= '" + selectMonth+"'"
            ResultSet Rs = stmt.executeQuery(sql);
