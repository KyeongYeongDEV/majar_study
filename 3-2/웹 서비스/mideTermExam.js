...
function calculateTotalScores(semesters) {
    const total = {}; // 과목별 점수를 합산해 저장할 객체

    // 각 학기별로 반복
    semesters.forEach(semester => {
    // 학기 객체를 [과목, 점수]의 배열로 변환하여 반복
    Object.entries(semester).forEach(([subject, score]) => {
        // 만약 점수가 100점이면 2배로 처리
        const adjustedScore = score === 100 ? score * 2 : score;
        // 해당 과목 점수 누적: 이미 있는 점수에 더하거나, 없으면 0에서 시작
        total[subject] = (total[subject] || 0) + adjustedScore;
    });
    });

    // 모든 과목의 총 점수를 가진 객체 반환
    return total;
}

function calculateAverage(semesters) {
    // 각 학기 객체의 값(점수)을 배열로 풀어냄 (2차원 배열 -> 1차원 배열)
    const totalScores = semesters.flatMap(Object.values);
    // 점수들의 총합 계산
    const totalSum = totalScores.reduce((acc, score) => acc + score, 0);
    // 총합을 점수 개수로 나누어 평균 반환
    return totalSum / totalScores.length;
}

// students 배열에서 평균 점수 80 이상인 학생만 필터링하고, 이름순 정렬
const result = students
    .filter(student => calculateAverage(student.semesters) >= 80) // 평균 점수 80 이상인 학생만 남김
    .map(student => ({
        // 각 학생의 이름과 과목별 총 점수를 포함한 객체 생성
        name: student.name,
        totalScores: calculateTotalScores(student.semesters)
    }))
